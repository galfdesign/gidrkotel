...
function quickSpecificLossPaPerM(diam, g) {
  // базовые справочные точки (по реальным графикам для PEX/металлопластика, сильно сглажены)
  // Скорости здесь в «комфортном» диапазоне 0.2–0.5 м/с, поэтому удельные потери заметно ниже,
  // чем в старой версии (где были сильно завышены).
  //
  // Примеры целевых точек:
  //  - Ø12:  G ≈ 0.17 м³/ч → ~270 Па/м  (калибровано под пример: 2 кВт, 12 мм, ΔT=10 K)
  //  - Ø16:  G ≈ 0.20 м³/ч → ~80 Па/м
  //  - Ø20:  G ≈ 0.40 м³/ч → ~40 Па/м
  //  - Ø25:  G ≈ 0.80 м³/ч → ~20 Па/м
  const ref = {
    12: { g: 0.17, pa: 270 },
    16: { g: 0.20, pa: 80 },
    20: { g: 0.40, pa: 40 },
